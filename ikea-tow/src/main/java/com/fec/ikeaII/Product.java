...
public class Product {
	...
	public void toFile(String diypath) {
		try {
			Writer writer = new BufferedWriter( new OutputStreamWriter(new FileOutputStream(diypath + product_id + ".html"),"utf-8"));   
	...
	public void toPic(int p, String diypath) {
			...
			try {
				for (int i = 0; i < pic_id.length; i++) {
					...
					FileOutputStream fos = new FileOutputStream(diypath + product_id + pic_id[i] + "_S" + p + ".tbi");
	...
	public void toCSV(String diypath) {
		File csvfile=new File(diypath+"//"+product_id+".csv");

		try{
			Writer writer = new BufferedWriter( new OutputStreamWriter(new FileOutputStream(csvfile),"gbk"));   
	
				writer.write(getDescribtion().toString());
	...
	public Product(String id) {
		buf = CaptureHtml.captureHtml(id);
		GetAnything something = new GetAnything();
